...
~~If the checksum doesn't match, that probably means that they released a new version. See the error that `xbps-src` reports at the end of the failed packaging and correct the checksum with the one that `xbps-src` gives right before it exits (`checksum=0123456789abcdef`).~~ Fixed, now the template fetches the distfiles through `do_fetch()` which doesn't require checksumming.
